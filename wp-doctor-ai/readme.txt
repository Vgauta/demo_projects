...
The plugin works without AI APIs. No external AI provider is contacted by default. If a site administrator enables a future AI provider integration, the plugin should send only the structured issue data needed to generate explanations or summaries.
...
No. The core plugin works without AI APIs. Diagnostics are deterministic and run inside WordPress and the browser session. AI features are optional and are intended only for explanations, summaries, translations, and reports.

= Is user data sent externally? =

No external service is contacted by default. Diagnostic data is stored locally in the WordPress database. If a site administrator enables a future AI provider integration, they should review what structured issue data is sent to that provider.
