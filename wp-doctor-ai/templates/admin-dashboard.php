...
$wpda_model = sanitize_text_field($wpda_settings['gemini_model'] ?? 'gemini-1.5-flash');
...
<div class="wrap wpda-shell" id="wp-doctor-ai-app">
    <header class="wpda-hero">
        ...
        <div class="wpda-credit-card">
            ...
            <?php if ($data['is_premium']) : ?>
                <small><?php esc_html_e('Premium: unlimited scans', 'wp-doctor-ai'); ?></small>
            <?php else : ?>
    ...
    <main class="wpda-layout">
        <section class="wpda-panel">
            <div class="wpda-panel-head">
                ...
                <div class="wpda-panel-actions">
                    ...
                    <label class="wpda-language-picker">
                        <span><?php esc_html_e('Any other language', 'wp-doctor-ai'); ?></span>
                        <input type="text" data-wpda-custom-language value="<?php echo esc_attr($wpda_custom_language); ?>" placeholder="<?php esc_attr_e('Example: Arabic, Tamil, Portuguese', 'wp-doctor-ai'); ?>" />
                    </label>
        ...
        <aside class="wpda-panel wpda-side">
            ...
            <?php endif; ?>
            <h3><?php esc_html_e('Premium AI setup', 'wp-doctor-ai'); ?></h3>
            <p><?php esc_html_e('Premium one-click solutions use Google AI Studio / Gemini to generate a safe, review-first action plan from the deterministic issue data. Add your API key before using AI fixes.', 'wp-doctor-ai'); ?></p>
            <?php if (isset($_GET['wpda_saved'])) : ?>
                <div class="notice notice-success inline"><p><?php esc_html_e('WP Doctor AI settings saved.', 'wp-doctor-ai'); ?></p></div>
            <?php endif; ?>
            <form class="wpda-settings-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
                <?php wp_nonce_field('wp_doctor_ai_settings'); ?>
                <input type="hidden" name="action" value="wp_doctor_ai_save_settings" />
                <label>
                ...
                <label>
                    ...
                    <input type="text" name="gemini_model" value="<?php echo esc_attr($wpda_model); ?>" />
                </label>
                <label>
            ...
            <div class="wpda-ai-help">
                <strong><?php esc_html_e('How to get a Google AI Studio API key', 'wp-doctor-ai'); ?></strong>
                <ol>
                    <li><?php esc_html_e('Open Google AI Studio and sign in with your Google account.', 'wp-doctor-ai'); ?></li>
                    <li><?php esc_html_e('Click Get API key, then create or select a project.', 'wp-doctor-ai'); ?></li>
                    <li><?php esc_html_e('Copy the key and paste it here. Keep it private and do not share it publicly.', 'wp-doctor-ai'); ?></li>
                </ol>
                <p><a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer"><?php esc_html_e('Open Google AI Studio API keys', 'wp-doctor-ai'); ?></a></p>
            </div>
