...
- [ ] Free ZIP contains a single top-level `wp-doctor-ai/` directory.
- [ ] Pro ZIP, if distributed separately, contains a single top-level `wp-doctor-ai-pro/` directory and requires the free plugin.
...
5. Rescue Credits and Premium AI setup panel.
...
- [ ] Google AI Studio / Gemini usage is clearly disclosed and remains opt-in with an administrator-provided API key.
