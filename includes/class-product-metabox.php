...
class CWPC_Product_Metabox {
	...
	public function save( $product_id ) {
		...
		$clean = array( 'table_colors' => array(), 'chair_designs' => array(), 'extra_chairs' => array(), 'chair_colors' => array(), 'chair_color_images' => array(), 'addons' => array() );
		foreach ( $schema['table_colors'] ?? array() as $row ) { $clean['table_colors'][] = array( 'name' => sanitize_text_field( $row['name'] ?? '' ), 'color' => sanitize_hex_color( $row['color'] ?? '' ), 'price' => (float) ( $row['price'] ?? 0 ), 'preview' => esc_url_raw( $row['preview'] ?? '' ), 'preview_id' => absint( $row['preview_id'] ?? 0 ) ); }
		foreach ( $schema['chair_designs'] ?? array() as $row ) { $clean['chair_designs'][] = array( 'name' => sanitize_text_field( $row['name'] ?? '' ), 'thumbnail' => esc_url_raw( $row['thumbnail'] ?? '' ), 'thumbnail_id' => absint( $row['thumbnail_id'] ?? 0 ), 'preview' => esc_url_raw( $row['preview'] ?? '' ), 'preview_id' => absint( $row['preview_id'] ?? 0 ), 'price' => (float) ( $row['price'] ?? 0 ), 'colors' => array_map( 'sanitize_text_field', (array) ( $row['colors'] ?? array() ) ) ); }
		...
		foreach ( $schema['chair_colors'] ?? array() as $row ) { $clean['chair_colors'][] = array( 'name' => sanitize_text_field( $row['name'] ?? '' ), 'color' => sanitize_hex_color( $row['color'] ?? '' ), 'price' => (float) ( $row['price'] ?? 0 ), 'preview' => esc_url_raw( $row['preview'] ?? '' ), 'preview_id' => absint( $row['preview_id'] ?? 0 ) ); }
		foreach ( $schema['chair_color_images'] ?? array() as $row ) { $clean['chair_color_images'][] = array( 'design' => sanitize_text_field( $row['design'] ?? '' ), 'color' => sanitize_text_field( $row['color'] ?? '' ), 'preview' => esc_url_raw( $row['preview'] ?? '' ), 'preview_id' => absint( $row['preview_id'] ?? 0 ) ); }
