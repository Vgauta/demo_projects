...
class CWPC_Assets {
	...
	public function admin( $hook ) {
		...
		if ( $screen && in_array( $screen->id, array( 'cwpc_configurator', 'product' ), true ) ) {
			wp_enqueue_media();
			wp_enqueue_style( 'cwpc-admin', CWPC_URL . 'assets/css/admin.css', array(), CWPC_VERSION );
			wp_enqueue_script( 'cwpc-admin-builder', CWPC_URL . 'assets/js/admin-builder.js', array( 'jquery' ), CWPC_VERSION, true );
		}
	}
