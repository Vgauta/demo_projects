...
class CWPC_Order {
	...
	public function meta( $item, $cart_item_key, $values, $order ) {
		...
		$item->add_meta_data( '_cwpc_configuration_json', wp_json_encode( $values['cwpc_configuration'] ), true );
		foreach ( (array) ( $values['cwpc_configuration']['summary'] ?? array() ) as $row ) { $item->add_meta_data( sanitize_text_field( $row['label'] ?? 'Configurator option' ), sanitize_text_field( $row['value'] ?? '' ), false ); }
